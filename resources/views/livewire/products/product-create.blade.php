<div>
    ...
    <flux:heading size="xl" level="1">{{ __('Create Products') }}</flux:heading>
    <flux:subheading size="lg" class="mb-6">{{ __('Create your Products') }}</flux:subheading>
        ...
        <div class="w-150">
                ...
                <option class="text-sm" value="">Select Category</option>
                    @foreach($categories as $category)
                        <option class="text-sm" value="{{ $category->id }}">{{ $category->category_name }}</option>
                    @endforeach
                </select>
